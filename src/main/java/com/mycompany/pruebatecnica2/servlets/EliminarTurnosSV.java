...
public class EliminarTurnosSV extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet EliminarTurnosSV</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet EliminarTurnosSV at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        // Obtener el ID del turno a eliminar del parámetro de la solicitud
        Long idTurno = Long.parseLong(request.getParameter("idTurno"));

        
        controladora.eliminarTurno(idTurno);

        response.sendRedirect("mensajeOkEliminarTurno.jsp");
    }
